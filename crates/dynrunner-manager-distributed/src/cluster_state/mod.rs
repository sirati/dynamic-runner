...
// Re-export the public-facing value types so external callers continue
// to see `cluster_state::TaskState`, `cluster_state::ApplyOutcome`, etc.,
// at the original paths. Sub-module-private items (`PeerState`,
// `PeerEntry`) stay `pub(super)` inside `types`.

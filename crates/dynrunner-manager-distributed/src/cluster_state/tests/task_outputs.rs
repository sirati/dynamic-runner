...
fn outputs_for_unknown_task_id_is_none() {
    ...
    assert!(s.outputs_for(&dynrunner_core::PhaseId::from("p0"), "anything").is_none());
}
...
fn task_completed_populates_task_outputs_cache() {
    ...
    assert_eq!(s.outputs_for(&dynrunner_core::PhaseId::from("p0"), "a"), Some(&outputs));
}
...
fn task_completed_with_no_result_data_does_not_populate() {
    ...
    assert!(s.outputs_for(&dynrunner_core::PhaseId::from("p0"), "a").is_none());
}
...
fn task_completed_malformed_result_data_stores_empty_outputs() {
    ...
    assert_eq!(outcome, ApplyOutcome::Applied);
    assert_eq!(s.outputs_for(&dynrunner_core::PhaseId::from("p0"), "a"), Some(&TaskOutputs::default()));
}
...
fn task_outputs_round_trip_via_snapshot() {
    ...
    joiner.restore(snap);
    assert_eq!(joiner.outputs_for(&dynrunner_core::PhaseId::from("p0"), "a"), Some(&outputs));
}
...
fn restore_first_write_wins_on_task_outputs_collision() {
    ...
    assert_eq!(local.outputs_for(&dynrunner_core::PhaseId::from("p0"), "a"), Some(&local_outputs));
}
...
fn python_encode_full_wrapper_decodes_outputs() {
    ...
    let cached = s.outputs_for(&dynrunner_core::PhaseId::from("p0"), "a").expect("cache populated");
    assert_eq!(
        cached.0.get("nonce"),

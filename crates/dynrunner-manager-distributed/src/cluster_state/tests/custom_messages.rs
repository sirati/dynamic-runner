...
//! sticky lattice (`absent ⊑ Unhandled ⊑ Handled`), the payload-drop
//! tombstone, the per-origin contiguous-prefix watermark compaction,
//! and the snapshot round-trip THROUGH the wire JSON (the #358
//! tuple-keyed-map lesson — a tuple-keyed snapshot field that
...
fn lattice_converges_in_all_arrival_orders_and_drops_payload() {
    ...
    for order in orders {
        ...
        assert_eq!(s.custom_handled_watermark("sec-1"), None);
    }
}
...
fn watermark_compacts_contiguous_handled_prefix_and_subsumes_replays() {
    ...
    assert_eq!(s.custom_handled_watermark("sec-1"), Some(1));
    ...
    s.apply(handled("sec-1", 2));
    assert_eq!(s.custom_handled_watermark("sec-1"), Some(4));
...
fn origins_are_independent_and_read_surface_is_sorted() {
    ...
    s.apply(handled("sec-1", 1));
    assert_eq!(s.custom_handled_watermark("sec-1"), Some(1));
    assert_eq!(s.custom_handled_watermark("sec-2"), None);
    assert_eq!(
        s.unhandled_custom_messages(),
        vec![
...
fn snapshot_with_nonempty_custom_messages_survives_json_and_restores() {
    ...
    assert_eq!(cold.custom_handled_watermark("sec-1"), Some(1));
    assert_eq!(
...
fn restore_merges_latch_and_watermark_over_local_state() {
    ...
    assert_eq!(a.custom_handled_watermark("sec-1"), Some(2));
    ...
    assert_eq!(b.custom_handled_watermark("sec-1"), Some(2));
    assert_eq!(
    ...
    assert_eq!(
        da.custom_handled_watermarks_hash,
        db.custom_handled_watermarks_hash
    );
}

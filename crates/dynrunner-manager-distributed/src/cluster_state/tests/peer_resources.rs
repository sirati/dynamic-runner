//! Tests for the `PeerResourceHoldingsUpdated` apply rule and the
//! peer-holdings snapshot round-trip.
//!
//! Pins:
//!
//!   - First-time announce inserts the holdings set; wire Vec<String>
//!     collects to a HashSet<String> for set-based equality / dedup.
//!   - Stale-epoch announces (epoch < primary_epoch) are silently
//!     dropped.
//!   - Unchanged-set re-announces are silent NoOps.
//!   - The peer_holdings map round-trips through snapshot/restore
//!     under the first-bootstrap-only contract (replace if local
//!     empty, otherwise keep local).

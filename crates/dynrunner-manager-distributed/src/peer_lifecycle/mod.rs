//! Peer-lifecycle dispatch module.
//!
//! Three concerns, one per submodule:
//!
//! - [`event`] defines the [`PeerLifecycleEvent`] value type that flows
//!   from the apply path (`ClusterState::emit_lifecycle_event`) across
//!   the dispatcher channel to consumers.
//! - [`listener`] defines the [`LifecycleListener`] trait every consumer
//!   (Rust-side respawn handler, PyO3 bridge adapter, future telemetry
//!   tap) implements. The trait is intentionally synchronous + fast:
...
pub use dispatcher::run_peer_lifecycle_dispatcher;
pub use event::{PeerLifecycleEvent, RemovalCause};

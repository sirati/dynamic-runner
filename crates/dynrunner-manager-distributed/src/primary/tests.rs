...
// ── preferred-secondaries validation integration tests ────────────
//
// These exercise the coordinator-level wiring of
// `PreferredSecondariesValidator`:
//   * `seed_cluster_state` calls `validate` once the seed batch
//     applies, against `self.secondaries.keys()` as the known set.
//   * `handle_cluster_mutation` re-validates AFTER a batch carrying
//     `PeerJoined` applies — the joined id is `forget`-ed from the
//     dedup set first so the next walk treats the id as fresh.
//
// We bypass `run()` because we only care about the two helpers'
// invariants — pool / worker plumbing is irrelevant. `NoPeers` for
// the peer transport and `setup_test(0)` for the secondary transport

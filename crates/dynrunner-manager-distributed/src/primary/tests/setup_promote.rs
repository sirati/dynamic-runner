...
/// The owner's discovery↔initial-assignment ordering (asm-tokenizer
/// parks-at-tasks=0 regression): a discovery-`Owed` primary must NOT mark
/// initial assignment complete — latch `run_start_batch_fired`, fan the
/// transfer-complete gate-release, fire the "initial setup done" milestone —
/// until discovery has SEEDED the ledger. The bring-up pre-loop DEFERS the
/// whole tail on the `Owed` path; `finish_discovery_arm` runs that SAME tail
/// (`finalize_bringup_assignment`) AFTER the seed. This pins that the latch
/// flips in the arm, never over the empty pre-discovery ledger (the empty
/// ledger marked complete + transfer-complete fired prematurely → the fleet
/// went operational with zero work and an incremental run never advanced).
#[tokio::test(flavor = "current_thread")]
async fn discovery_owed_defers_initial_assignment_until_arm_seeds() {
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let (transport, _ends) = setup_test(1);
            ...
            // A relocated mode-2 primary: discovery is Owed and the bring-up
            // kind is a relocation (the kind the pre-loop stamps onto `self`).
            primary
                .cluster_state_mut_for_test()
                .apply(ClusterMutation::DiscoveryDebtDeclared);
            primary.bringup_kind = crate::process::BootstrapKind::BootstrapRelocation;

            // PRE-SEED: the run-start batch has NOT fired. The empty
            // pre-discovery ledger must not be marked "assignment complete".
            assert!(
                !primary.run_start_batch_fired,
                "run_start_batch_fired must be false while discovery is Owed (pre-seed)"
            );

            // The on-resolve arm seeds the ledger (flips Owed → Settled), then
            // runs the deferred bring-up tail.
            let t1 = make_binary("disc-1", 100);
            let h1 = compute_task_hash(&t1);
            primary
                .finish_discovery_arm(vec![(t1, false)], HashMap::new(), &mut None)
                .await
                .expect("Owed arm seeds + runs the deferred bring-up cleanly");

            assert_eq!(
                primary.cluster_state_for_test().discovery_debt(),
                DiscoveryDebt::Settled,
                "the arm settles discovery atomically with the seed"
            );
            assert_eq!(
                primary.cluster_state_for_test().task_count(),
                1,
                "the discovered task is seeded before the bring-up tail runs"
            );
            assert!(
                primary.cluster_state_for_test().task_state(&h1).is_some(),
                "the discovered hash is present in the seeded ledger"
            );
            // POST-SEED: the deferred bring-up tail ran in the arm — the
            // run-start batch is now latched (the "initial setup done"
            // milestone + transfer-complete fired HERE, over the seeded ledger,
            // not over the empty pre-discovery ledger).
            assert!(
                primary.run_start_batch_fired,
                "finish_discovery_arm must run the deferred bring-up tail (latch \
                 the run-start batch) AFTER seeding the ledger"
            );
        })
        .await;
}

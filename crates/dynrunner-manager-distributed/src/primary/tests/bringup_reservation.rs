...
//! member's view to its reserved share, so each member drains only its ~2
//! reserved tasks and never the 14/14/0 pack. This partitioning is
//! mesh-INDEPENDENT: it caps what ANY member pulls, with no `MeshReady`
//! involvement. (The reservation once ALSO leaned on the
//! `should_skip_worker_for_dispatch` mesh-veto to withhold sends to
//! unconfirmed members; that veto is removed — dispatch is decoupled from
//! the peer mesh and the strand concern is covered by the secondary's
//! confirmable-replay — so a member's reserved share now dispatches as
//! soon as it has idle workers, regardless of `MeshReady`.)
//!
//! Deterministic direct-handler tests: register the roster + reservation,
//! run the `TasksAdded` recheck, and assert each member's wire receives
//! only its reserved share (the even spread), independent of MeshReady.
...
/// THE HEADLINE REPRO — the surviving (mesh-INDEPENDENT) contract. 15
/// members × 2 workers, 28-task cold pool. The bring-up reservation
/// PARTITIONS the pool so every member dispatches ONLY its reserved share
/// (≤ ceil(28/15)=2 each) — never the production 14/14/0×13 pack — even
/// when the whole batch is dispatched in ONE recheck. This is the pool
/// partitioning alone; it does NOT depend on `MeshReady` (dispatch is
/// decoupled from the peer mesh). Members that never reported `MeshReady`
/// still receive their reserved share, so the final spread is even (1-2
/// per member) and the full pool dispatches with nothing stranded.
#[tokio::test(flavor = "current_thread")]
async fn reservation_caps_spread_to_per_member_share_without_mesh_ready() {
    ...
    local
        .run_until(async {
            ...
            // NONE of sec-2..sec-14 ever reported MeshReady — irrelevant to
            // dispatch now (the veto is gone), but pin that the reservation
            // partitioning still caps their shares without it.
            ...
            // ONE recheck: the whole pool dispatches, but the reservation
            // caps each member to its reserved share — no 14/14/0 pack,
            // even though 13 members never reported MeshReady.
            ...
            assert_eq!(
                total, 28,
                "the whole 28-task pool must dispatch — every member (confirmed \
                 or not) receives its reserved share; per-member: {per_member:?}"
            );
            ...
            assert!(
                ...
                "28 tasks over 15 members must spread to 1..=2 each — the \
                 reservation partitioning prevents the production 14/14/0×13 \
                 pack, mesh-independently; per-member: {per_member:?}"
            );
            assert_eq!(
...
/// REDISTRIBUTE. Two members are DECLARED DEAD (the genuine
/// member-removal path) after the reservation dispatched their shares.
/// Their in-flight tasks are recovered to the pool and fold onto the
/// surviving fleet — no strand, no permanent idle pool. CASCADE: two
/// members die; both shares fold into the survivors. (The redistribution
/// is mesh-independent: it keys on member DEATH, not MeshReady.)
#[tokio::test(flavor = "current_thread")]
async fn dead_member_share_redistributes_onto_survivors() {
    ...
    local
        .run_until(async {
            ...
            // recheck drains the whole redistributed share.
            seed_roster_and_tasks(&mut primary, n_members, 8, 8);

            primary.seed_bringup_reservation(crate::process::BootstrapKind::BootstrapRelocation);
            assert!(primary.pool().reservation_active());

            // First recheck: the reservation partitions the 8-task pool 2
            // per member across ALL FOUR members (dispatch is decoupled
            // from MeshReady — every member with idle workers takes its
            // reserved share).
            run_dispatch_recheck(&mut primary).await;
            settle_pump().await;
            let confirmed_first: usize =
                assigned_ids(&mut ends[0].1).len() + assigned_ids(&mut ends[1].1).len();
            let doomed_first: usize =
                assigned_ids(&mut ends[2].1).len() + assigned_ids(&mut ends[3].1).len();
            assert_eq!(
                confirmed_first + doomed_first,
                8,
                "the reservation spreads all 8 tasks 2-per-member across the \
                 four members; survivors={confirmed_first} doomed={doomed_first}"
            );
            assert_eq!(
                primary.pool().iter().count(),
                0,
                "the whole partitioned pool dispatched in one recheck"
            );

            // sec-2 dies → declared dead (the genuine member-removal path).
            // Its in-flight tasks recover to the pool and fold onto the
            // survivors.
            primary
                .requeue_dead_secondary_for_test("sec-2")
                .await
                .expect("dead-member cleanup");
            // sec-3 ALSO dies (cascade) → its in-flight tasks fold onto the
            ...
            // After both deaths the recovered tasks redistribute to the
            ...
            assert_eq!(
                survivor_total, 8,
                "every task — including the two dead members' recovered \
                 shares — ends up dispatched onto the survivors; nothing stranded"
            );
            assert_eq!(
                primary.pool().iter().count(),
                0,
                "no task strands after the dead members' shares redistribute"
            );
        })
        .await;
}

// NOTE: the former #382 test
// `unconfirmed_member_with_reserved_share_is_never_sent_an_assignment` is
// REMOVED. It pinned the mesh-VETO half of the bring-up reservation — a
// member without a landed `MeshReady` was never sent its reserved share.
// That veto is gone (dispatch is decoupled from the peer mesh; the strand
// concern it guarded is covered by the secondary's confirmable-replay), so
// a member's reserved share now dispatches as soon as it has idle workers,
// regardless of `MeshReady`. The reservation's surviving VALUE — the
// mesh-INDEPENDENT pool PARTITIONING that caps each member's share and
// prevents the 14/14/0 pack — is exercised by the spread tests below.

...
/// Simulate a secondary that sends welcome + cert, completes
/// initial-assigned tasks, and — once promoted via `FullTaskList` —
/// also completes every still-pending task on the primary's behalf
/// (simulating the SLURM-primary's authoritative dispatch path).
///
/// The post-handoff behaviour is required because the real primary
/// now demotes itself the moment it sends `PromotePrimary` and stops
/// dispatching; the fake stands in for the secondary-side
/// SLURM-primary that would otherwise drain the run via
/// `slurm_pending`. Without it, tests that rely on more binaries than
/// fit in the initial assignment would hang waiting for completions
/// that no longer come from the local primary.

//! Integration test for the panik (operator-initiated emergency
//! stop) end-to-end wire path on the secondary.
//!
//! Single concern of this file: pin the pipeline
//! "watcher signal arrives → coordinator broadcasts
//! `ClusterMutation::PanikRequested` → coordinator returns
...
//! emission AND the `RunOutcome` returned to the caller.
...
/// Run a secondary against a fake primary, register a panik oneshot
/// receiver, fire the panik signal once the loop is in
/// `process_tasks`, and assert:
...
async fn panik_signal_broadcasts_and_returns_panik_shutdown() {

    ...
    pub(super) async fn process_tasks(
        ...
        loop {
                ...
                && !self.cluster_state.run_complete()
            {
                ...
                // exit: 2 s is generous on a TCP socket under SSH
                // tunnel latency and well within any caller-side
                // run timeout; the alternative (unbounded await)
                // would re-trade one hang for another.

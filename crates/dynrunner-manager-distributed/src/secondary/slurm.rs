...
use dynrunner_core::{Identifier, PhaseId, TaskInfo, TypeId, WorkerId};
    ...
    pub(super) fn populate_slurm_tasks(
        ...
        let mut items: Vec<TaskInfo<I>> = all_tasks
            ...
            .map(|task| {
                ...
                // TODO(phase-4b): wire phase_id/type_id/affinity_id/payload
                // through TaskListEntry / DistributedBinaryInfo so the
                // SLURM-primary preserves the original bucketing instead
                // of collapsing every item onto the "default" bucket.
                TaskInfo {
                    path: binary_path,

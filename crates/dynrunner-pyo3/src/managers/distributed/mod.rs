...
pub(crate) struct PyDistributedManager {
    ...
    /// (#577) The `import_action` field is GONE — SecondaryAffine gate
    /// bodies now run in worker subprocesses dispatched via the normal
    /// task-dispatch path; the consumer registers a `TaskTypeSpec` whose

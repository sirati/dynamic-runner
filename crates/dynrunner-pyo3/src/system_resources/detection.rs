//! Runtime detection of cpu/memory limits, with cgroup v2 awareness.
//!
//! Three public entry points used by the `system_resources` module:
//!   - `detect_logical_cpu_count() -> u32`
//!   - `detect_total_memory_bytes() -> u64`
//!   - `detect_available_memory_bytes() -> u64`
//!
...
//! both ignore cgroup quotas. The detection helpers clamp the host
//! values against `cpu.max` / `memory.max` walked from the leaf
//! cgroup upward (the kernel enforces the tightest constraint in
//! the chain, so a single ancestor with a tighter cap wins). cgroup
//! v1 hosts and non-Linux platforms see no clamp —
//! `cgroup_v2_*_limit()` returns `None`.
